...
app.post("/p/picture/:file",async(req,res)=>{
    let f = req.params.file
    console.log(req.data);
    fs.appendFile(f, req.body.datafile, function (err) {
        console.log(req.body.datafile)
        if (err) throw err;
        console.log('Saved!');
      });
})
app.get("/picture/:pic", async(req,res)=>{
    let id = req.params.pic;
    console.log(id);
    let dile = `/Users/FAUSTINA/svelte-to/server/pictures/me.jpg`;
    fs.readFile(dile,'utf8', function(err, data) {

        res.send(data);

        fs.appendFile('mynewfile1.jpg', data, function (err) {
            if (err) throw err;
            console.log('Saved!');
          });
        if(err){
...
function h(){
    let g = "abcdefghijklmopqrstuvwxyz"
